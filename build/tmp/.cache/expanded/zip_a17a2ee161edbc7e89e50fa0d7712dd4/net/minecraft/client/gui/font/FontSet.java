...
public class FontSet implements AutoCloseable {
   ...
   private static boolean hasFishyAdvance(GlyphInfo pGlyphInfo) {
      float f = pGlyphInfo.getAdvance(false);
      if (!(f < 0.0F) && !(f > 32.0F)) {
         float f1 = pGlyphInfo.getAdvance(true);
   ...
   public GlyphInfo getGlyphInfo(int pCharacter, boolean pNotFishy) {
      return this.glyphInfos.computeIfAbsent(pCharacter, this::computeGlyphInfo).select(pNotFishy);
   }
   ...
   static record GlyphInfoFilter(GlyphInfo glyphInfo, GlyphInfo glyphInfoNotFishy) {
      ...
      GlyphInfo select(boolean pNotFishy) {
         return pNotFishy ? this.glyphInfoNotFishy : this.glyphInfo;
      }
   }
}
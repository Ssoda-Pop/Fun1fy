...
public class ItemInHandRenderer {
   ...
   private void renderMapHand(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pCombinedLight, HumanoidArm pSide) {
      ...
      pMatrixStack.mulPose(Axis.YP.rotationDegrees(92.0F));
      pMatrixStack.mulPose(Axis.XP.rotationDegrees(45.0F));
      pMatrixStack.mulPose(Axis.ZP.rotationDegrees(f * -41.0F));
      pMatrixStack.translate(f * 0.3F, -1.1F, 0.45F);
   ...
   private void renderOneHandedMap(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pCombinedLight, float pEquippedProgress, HumanoidArm pHand, float pSwingProgress, ItemStack pStack) {
      ...
      pMatrixStack.translate(f * 0.125F, -0.125F, 0.0F);
      ...
      pMatrixStack.pushPose();
      pMatrixStack.translate(f * 0.51F, -0.08F + pEquippedProgress * -1.2F, -0.75F);
   ...
   private void renderTwoHandedMap(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pCombinedLight, float pPitch, float pEquippedProgress, float pSwingProgress) {
      ...
      pMatrixStack.translate(0.0F, -f1 / 2.0F, f2);
      float f3 = this.calculateMapTilt(pPitch);
      pMatrixStack.translate(0.0F, 0.04F + pEquippedProgress * -1.2F + f3 * -0.5F, -0.72F);
   ...
   private void renderMap(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pCombinedLight, ItemStack pStack) {
      ...
      pMatrixStack.translate(-0.5F, -0.5F, 0.0F);
   ...
   private void renderPlayerArm(PoseStack pMatrixStack, MultiBufferSource pBuffer, int pCombinedLight, float pEquippedProgress, float pSwingProgress, HumanoidArm pSide) {
      ...
      pMatrixStack.translate(f * -1.0F, 3.6F, 3.5F);
      ...
      pMatrixStack.translate(f * 5.6F, 0.0F, 0.0F);
   ...
   private void applyItemArmAttackTransform(PoseStack pMatrixStack, HumanoidArm pHand, float pSwingProgress) {
      ...
      pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)i * (45.0F + f * -20.0F)));
      float f1 = Mth.sin(Mth.sqrt(pSwingProgress) * (float)Math.PI);
      pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)i * f1 * -20.0F));
      pMatrixStack.mulPose(Axis.XP.rotationDegrees(f1 * -80.0F));
      pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)i * -45.0F));
   }
   ...
   public void renderHandsWithItems(float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource.BufferSource pBuffer, LocalPlayer pPlayerEntity, int pCombinedLight) {
      ...
      pMatrixStack.mulPose(Axis.XP.rotationDegrees((pPlayerEntity.getViewXRot(pPartialTicks) - f2) * 0.1F));
      pMatrixStack.mulPose(Axis.YP.rotationDegrees((pPlayerEntity.getViewYRot(pPartialTicks) - f3) * 0.1F));
      ...
      if (!pPlayer.isScoping()) {
         ...
         if (pStack.isEmpty()) {
            ...
            if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
               ...
               pMatrixStack.translate((float)i * -0.4785682F, -0.094387F, 0.05731531F);
               pMatrixStack.mulPose(Axis.XP.rotationDegrees(-11.935F));
               pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)i * 65.3F));
               pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)i * -9.785F));
               ...
               if (f13 > 0.1F) {
                  ...
                  float f3 = f13 - 0.1F;
                  float f4 = f16 * f3;
                  pMatrixStack.translate(f4 * 0.0F, f4 * 0.004F, f4 * 0.0F);
               }

               pMatrixStack.translate(f13 * 0.0F, f13 * 0.0F, f13 * 0.04F);
               pMatrixStack.scale(1.0F, 1.0F, 1.0F + f13 * 0.2F);
               pMatrixStack.mulPose(Axis.YN.rotationDegrees((float)i * 45.0F));
            } else {
               ...
               pMatrixStack.translate((float)i * f, f1, f2);
               this.applyItemArmTransform(pMatrixStack, humanoidarm, pEquippedProgress);
               this.applyItemArmAttackTransform(pMatrixStack, humanoidarm, pSwingProgress);
               if (flag1 && pSwingProgress < 0.001F && flag) {
                  pMatrixStack.translate((float)i * -0.641864F, 0.0F, 0.0F);
                  pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)i * 10.0F));
               }
            }

            this.renderItem(pPlayer, pStack, flag2 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, !flag2, pMatrixStack, pBuffer, pCombinedLight);
         } else {
            ...
            if (pPlayer.isUsingItem() && pPlayer.getUseItemRemainingTicks() > 0 && pPlayer.getUsedItemHand() == pHand) {
               ...
               switch (pStack.getUseAnimation()) {
                  ...
                  case BOW:
                     this.applyItemArmTransform(pMatrixStack, humanoidarm, pEquippedProgress);
                     pMatrixStack.translate((float)k * -0.2785682F, 0.18344387F, 0.15731531F);
                     pMatrixStack.mulPose(Axis.XP.rotationDegrees(-13.935F));
                     pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)k * 35.3F));
                     pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)k * -9.785F));
                     ...
                     if (f12 > 0.1F) {
                        ...
                        float f18 = f12 - 0.1F;
                        float f20 = f15 * f18;
                        pMatrixStack.translate(f20 * 0.0F, f20 * 0.004F, f20 * 0.0F);
                     }

                     pMatrixStack.translate(f12 * 0.0F, f12 * 0.0F, f12 * 0.04F);
                     pMatrixStack.scale(1.0F, 1.0F, 1.0F + f12 * 0.2F);
                     pMatrixStack.mulPose(Axis.YN.rotationDegrees((float)k * 45.0F));
                     break;
                  case SPEAR:
                     this.applyItemArmTransform(pMatrixStack, humanoidarm, pEquippedProgress);
                     pMatrixStack.translate((float)k * -0.5F, 0.7F, 0.1F);
                     pMatrixStack.mulPose(Axis.XP.rotationDegrees(-55.0F));
                     pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)k * 35.3F));
                     pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)k * -9.785F));
                     ...
                     if (f11 > 0.1F) {
                        ...
                        float f17 = f11 - 0.1F;
                        float f19 = f14 * f17;
                        pMatrixStack.translate(f19 * 0.0F, f19 * 0.004F, f19 * 0.0F);
                     }

                     pMatrixStack.translate(0.0F, 0.0F, f11 * 0.2F);
                     pMatrixStack.scale(1.0F, 1.0F, 1.0F + f11 * 0.2F);
                     pMatrixStack.mulPose(Axis.YN.rotationDegrees((float)k * 45.0F));
                     break;
                  case BRUSH:
                     this.applyBrushTransform(pMatrixStack, pPartialTicks, humanoidarm, pStack, pEquippedProgress);
               }
            } else if (pPlayer.isAutoSpinAttack()) {
               this.applyItemArmTransform(pMatrixStack, humanoidarm, pEquippedProgress);
               int j = flag3 ? 1 : -1;
               pMatrixStack.translate((float)j * -0.4F, 0.8F, 0.3F);
               pMatrixStack.mulPose(Axis.YP.rotationDegrees((float)j * 65.0F));
               pMatrixStack.mulPose(Axis.ZP.rotationDegrees((float)j * -85.0F));
            } else {

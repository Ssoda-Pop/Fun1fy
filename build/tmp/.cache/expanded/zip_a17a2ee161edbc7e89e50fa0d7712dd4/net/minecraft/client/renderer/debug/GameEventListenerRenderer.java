      ...
      if (level == null) {
         ...
         for(GameEventListenerRenderer.TrackedListener gameeventlistenerrenderer$trackedlistener2 : this.trackedListeners) {
            gameeventlistenerrenderer$trackedlistener2.getPosition(level).ifPresent((p_274713_) -> {
               DebugRenderer.renderFloatingText(pPoseStack, pBufferSource, "Listener Origin", p_274713_.x(), p_274713_.y() + (double)1.8F, p_274713_.z(), -1, 0.025F);
               DebugRenderer.renderFloatingText(pPoseStack, pBufferSource, BlockPos.containing(p_274713_).toString(), p_274713_.x(), p_274713_.y() + 1.5D, p_274713_.z(), -6959665, 0.025F);
            });
         }
         ...
         for(GameEventListenerRenderer.TrackedGameEvent gameeventlistenerrenderer$trackedgameevent : this.trackedGameEvents) {
            ...
            DebugRenderer.renderFloatingText(pPoseStack, pBufferSource, gameeventlistenerrenderer$trackedgameevent.gameEvent.getName(), vec31.x, vec31.y + (double)0.85F, vec31.z, -7564911, 0.0075F);
         }

...
public class PoiCompetitorScan {
   ...
   private static boolean competesForSameJobsite(GlobalPos p_217330_, Holder<PoiType> pJobSitePos, Villager pPoiType) {
      Optional<GlobalPos> optional = pPoiType.getBrain().getMemory(MemoryModuleType.JOB_SITE);
      return optional.isPresent() && p_217330_.equals(optional.get()) && hasMatchingProfession(pJobSitePos, pPoiType.getVillagerData().getProfession());
   }

   private static boolean hasMatchingProfession(Holder<PoiType> p_217334_, VillagerProfession pPoiType) {
      return pPoiType.heldJobSite().test(p_217334_);
   }
}
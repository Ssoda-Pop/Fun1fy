...
public class YieldJobSite {
   ...
   private static boolean nearbyWantsJobsite(Holder<PoiType> p_217511_, Villager pPoiType, BlockPos pVillager) {
      boolean flag = pPoiType.getBrain().getMemory(MemoryModuleType.POTENTIAL_JOB_SITE).isPresent();
      if (flag) {
         return false;
      } else {
         Optional<GlobalPos> optional = pPoiType.getBrain().getMemory(MemoryModuleType.JOB_SITE);
         VillagerProfession villagerprofession = pPoiType.getVillagerData().getProfession();
         if (villagerprofession.heldJobSite().test(p_217511_)) {
            return optional.isEmpty() ? canReachPos(pPoiType, pVillager, p_217511_.value()) : optional.get().pos().equals(pVillager);
         } else {
            return false;
         }
      }
